# Ubuntu Resolute Generic

This template builds a NoCloud cloud-init ISO for Ubuntu 24.04 LTS (Resolute).
...
- `create-env.sh`: interactive helper that collects host/network values, timezone, optional time server, passwords and SSH public keys, then writes `files/.env`
- `create-iso.sh`: renders `user-data`, `network-config`, and `meta-data`, then builds `image/cinit_UbuntuResolute_generic.iso`
